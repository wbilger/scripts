...
do
    FILE=$(find "${CACHE}/media/videos/Movies-Cartoons" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/videos/Movies-Cartoons/./${FILE}" "${BACKING}/media/videos/Movies-Cartoons" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/videos/Movies-Cartoons/./${FILE}" "${BACKING}/media/videos/Movies-Cartoons/" 
done
