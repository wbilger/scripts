...
do
    FILE=$(find "${CACHE}/media/videos/Movies" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/videos/Movies/./${FILE}" "${BACKING}/media/videos/Movies" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/videos/Movies/./${FILE}" "${BACKING}/media/videos/Movies/" 
done
