...
do
    FILE=$(find "${CACHE}/media/videos/Christmas" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/videos/Christmas/./${FILE}" "${BACKING}/media/videos/Christmas" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/videos/Christmas/./${FILE}" "${BACKING}/media/videos/Christmas/" 
done
