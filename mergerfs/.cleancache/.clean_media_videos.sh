...
do
    FILE=$(find "${CACHE}/media" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/./${FILE}" "${BACKING}/media/" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/./${FILE}" "${BACKING}/media/" 
done
