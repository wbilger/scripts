...
do
    FILE=$(find "${CACHE}/media/books" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/books/./${FILE}" "${BACKING}/media/books" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/books/./${FILE}" "${BACKING}/media/books/" 
done
