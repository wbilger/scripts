...
do
    FILE=$(find "${CACHE}/media/pictures" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/pictures/./${FILE}" "${BACKING}/media/pictures" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/pictures/./${FILE}" "${BACKING}/media/pictures/" 
done
