...
do
    FILE=$(find "${CACHE}/media/videos/Documentaries" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/videos/Documentaries/./${FILE}" "${BACKING}/media/videos/Documentaries" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/videos/Documentaries/./${FILE}" "${BACKING}/media/videos/Documentaries/" 
done
