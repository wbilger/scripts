...
do
    FILE=$(find "${CACHE}/tmp" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/tmp/./${FILE}" "${BACKING}/tmp/" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/tmp/./${FILE}" "${BACKING}/tmp/" 
done
