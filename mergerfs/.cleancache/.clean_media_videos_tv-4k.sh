...
do
    FILE=$(find "${CACHE}/media/videos/TV-4k" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/videos/TV-4k/./${FILE}" "${BACKING}/media/videos/TV-4k" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/videos/TV-4k/./${FILE}" "${BACKING}/media/videos/TV-4k/" 
done
