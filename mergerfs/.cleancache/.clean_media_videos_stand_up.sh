...
do
    FILE=$(find "${CACHE}/media/videos/Stand Up" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/videos/Stand Up/./${FILE}" "${BACKING}/media/videos/Stand Up" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/videos/Stand Up/./${FILE}" "${BACKING}/media/videos/Stand Up/" 
done
