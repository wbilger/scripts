...
do
    FILE=$(find "${CACHE}/media/music" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/music/./${FILE}" "${BACKING}/media/music" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/music/./${FILE}" "${BACKING}/media/music/" 
done
