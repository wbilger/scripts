...
do
    FILE=$(find "${CACHE}/media/videos/TV-Nature" -type f -printf '%A@ %P\n' | \
                  sort | \
                  head -n 1 | \
                  cut -d' ' -f2-)
    test -n "${FILE}"
    echo rsync "${CACHE}/media/videos/TV-Nature/./${FILE}" "${BACKING}/media/videos/TV-Nature" >> ${LOGFILE}
    rsync -axqHAXWESR --preallocate --remove-source-files "${CACHE}/media/videos/TV-Nature/./${FILE}" "${BACKING}/media/videos/TV-Nature/" 
done
